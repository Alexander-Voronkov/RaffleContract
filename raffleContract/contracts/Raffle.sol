...
contract Raffle is Initializable, OwnableUpgradeable, AutomationCompatibleInterface {
  ...
  function initialize(
    ...
    ExchangeFeedFetcher _feedFetcher
  ) public initializer {
  ...
  function addAllowedToken(address tokenAddress, address feedAddress) external onlyOwner {
  ...
  function deposit(address _tokenAddress, uint _amount) external {
    ...
    require(
      playersOfCurrentRaffle[msg.sender].depositedTokenAddress == address(0),
      "You already participate"
    );

    (int currency, uint8 decimals) = feedFetcher.getLatestData(
      tokensAllowedWithFeed[_tokenAddress]
    );
    ...
    uint amountToWithdraw = swapper.swapTokenForETH(
      _tokenAddress,
      _amount,
      address(this),
      address(this)
