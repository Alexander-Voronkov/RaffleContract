...
describe('VRF tests', async function () {
    ...
    const arbitrumAddress = '0x912CE59144191C1204E64559FE8253a0e49E6548';
    ...
    it('vrf is generating a random number', async () => {
        ...
        // 2. Получить requestId из события (если нужно)
        // Можно получить из receipt.events, если контракт эмитит событие с requestId

        // 3. Fulfill запрос вручную через мок
        // Обычно VRFMock имеет функцию типа fulfillRandomWords(requestId, consumerAddress)
        // Пример (замените на актуальное имя функции и параметры, если отличается):
        const requestId = await consumer.s_requestId();
        await mock.fulfillRandomWords(requestId, consumer);

        // 4. Проверить, что сгенерированы случайные слова
