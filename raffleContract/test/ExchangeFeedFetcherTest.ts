...
describe("ExchangeFeedTests", async () => {
  async function deployExchangeFeedModule() {
    ...
    const typedFetcher = ExchangeFeedFetcher__factory.connect(
  ...
  it("should fetch current prices", async function () {
    const { fetcher, owner, others } = await loadFixture(
      deployExchangeFeedModule,
    );

    const [currency1, decimals1] = await fetcher
      .connect(owner)
      .getLatestData(feeds.eth_usd_feed);
    const [currency3, decimals3] = await fetcher
      .connect(owner)
      .getLatestData(feeds.usdc_usd_feed);
    const [currency4, decimals4] = await fetcher
      .connect(owner)
      .getLatestData(feeds.usdt_usd_feed);
