...
import {
  ...
  usdcAddress,
  usdtAddress,
} from "../constants/tokensAddresses";
...
export default function WheelOfFortune() {
  ...
  const totalAmountInUsd = useMemo(() => {
    ...
    return players.reduce(
      (prev, curr) => prev + curr.amountDepositedInUsd,
      BigInt(0),
    );
  }, [players]);
  ...
  const deposit = useCallback(() => {
    ...
    const depositAmountConverted = BigInt(
  ...
  const depositAmountConverted = useMemo(() => {
    return BigInt(
      Math.floor(
        depositAmount * 10 ** (balances.get(selectedToken)?.decimals ?? 18),
      ),
    );
  ...
  const canDeposit = useMemo(
    () => depositAmount > 0 && depositAmount <= maxDeposit,
    [depositAmount, maxDeposit],
  );

  const playerColors = useMemo(
    () => players.map(() => randomColor()),
    [players],
  );
    ...
    >
      ...
      {showWinner && (
        <Alert severity="success" sx={{ mb: 3 }}>
          Победитель: {winner ? winner.slice(0, 6) : "—"}... выиграл{" "}
          {formatUnits(totalAmountInUsd, 14) + "USD"}
        </Alert>
      )}
      ...
      <Box display="flex" gap={6} flexWrap="wrap" justifyContent="center">
        <Box display="flex" flexDirection="column" alignItems="center" gap={4}>
              ...
              <svg viewBox="0 0 300 300" width="100%" height="100%">
                {players.length > 0 ? (
                  (() => {
                    let currentAngle = 0;
                    const radius = 150;
                    const totalStake = players.reduce(
                      (sum, p) => sum + p.stake,
                      0,
                    );
                    ...
                    return players.map((player, i) => {
                      ...
                      const x1 =
                        150 + radius * Math.cos((Math.PI / 180) * startAngle);
                      const y1 =
                        150 + radius * Math.sin((Math.PI / 180) * startAngle);
                      const x2 =
                        150 + radius * Math.cos((Math.PI / 180) * endAngle);
                      const y2 =
                        150 + radius * Math.sin((Math.PI / 180) * endAngle);

                      const midAngle = startAngle + segmentAngle / 2;
                      const labelX =
                        150 +
                        (radius - 40) * Math.cos((Math.PI / 180) * midAngle);
                      const labelY =
                        150 +
                        (radius - 40) * Math.sin((Math.PI / 180) * midAngle);
          ...
          >
            ...
            <Typography>
              Баланс: {maxDeposit.toFixed(4)}{" "}
              {balances.get(selectedToken)?.symbol}
            </Typography>
          ...
          {players.map((player, index) => (
            ...
            >
              <Box
                ...
                height={16}
                borderRadius="50%"
                bgcolor={playerColors[index]}
              />
              <Typography fontSize={14}>
                Игрок {index + 1}: {player.playerAddress.slice(0, 6)}... —{" "}
                {Number(
                  formatUnits(
                    player.depositedTokenAmount,
                    balances.get(player.depositedTokenAddress)?.decimals ?? 18,
                  ),
                ).toFixed(4)}{" "}
